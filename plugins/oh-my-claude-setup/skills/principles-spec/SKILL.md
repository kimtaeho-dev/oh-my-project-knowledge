...
1. `.claude/DOMAIN_MAP.md`에서 해당 기능의 파일 위치와 증상 키워드를 먼저 확인한다
2. 위치가 명시된 경우 해당 경로로 바로 이동한다
3. 명시되지 않은 경우에만 Glob/Grep으로 탐색한다
4. VOC/문의 대응 시에는 사용자 표현을 DOMAIN_MAP.md의 증상 키워드 열과 매칭하여 의심 영역을 좁힌다

## 코드 수정 전 영향 범위 확인

수정 대상 파일이 export하는 심볼/엔드포인트의 사용처를 코드에서 검색하여 파악한다.
사용처가 다른 도메인에 있으면 해당 코드를 직접 읽어 영향 범위를 판단한다.
판단이 어려우면 작업을 중단하고 사용자에게 영향 범위를 확인 요청한다.

영향 관계는 정적 매트릭스로 관리하지 않는다. 코드 변경 시점에 직접 추론한다.

## DOMAIN_MAP.md 갱신 의무

코드 수정으로 다음 변경이 발생하면 DOMAIN_MAP.md를 갱신한다:

- 새 기능/페이지/엔드포인트 추가 → 새 행 추가
- 파일 위치 이동 → 파일 위치 열 갱신
- 기능 제거 → 해당 행 삭제

증상 키워드는 실제 VOC/문의에서 새 표현이 발견될 때 사용자가 직접 추가한다.
